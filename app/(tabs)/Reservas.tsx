...
const ReservationScreen = () => {
  ...
  const fetchAvailableHours = async (date: string, employeeId: number) => {
    ...
    try {
      ...
      // Si la respuesta es un array, son las horas disponibles
      if (Array.isArray(response.data)) {
        console.log("✅ Horas disponibles:", response.data);
        setAvailableHours(response.data);
        setReservedHours([]); // No hay horas reservadas en este caso
      } 
      // Si la respuesta es un objeto con availableSlots
      else if (response.data?.availableSlots) {
        const { availableSlots = [], reservedSlots = [] } = response.data;
        console.log("✅ Horas disponibles:", availableSlots);
        console.log("❌ Horas reservadas:", reservedSlots);
        
        setAvailableHours(availableSlots);
        setReservedHours(reservedSlots);
      } 
  ...
  const handleConfirmReservation = async () => {
    if (!selectedDate || !selectedHour || !selectedEmployee) {
      console.warn("⚠️ Intento de reserva sin datos completos");
      Alert.alert("Error", "Seleccione fecha, hora y empleado.");
      return;
    }

    if (reservedHours.includes(selectedHour)) {
      console.warn(`⚠️ Intento de reservar hora ocupada: ${selectedHour}`);
      Alert.alert("Error", "Esta hora ya está reservada.");
      return;
    }
