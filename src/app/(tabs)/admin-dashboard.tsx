...
function EmployeeAppointmentsScreen() {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      try {
        ...
        if (!token || !userId) {
          showError('Por favor, inicie sesión nuevamente');
          router.push('/login');
          return;
        }
        ...
        if (userRole !== 'employee' && userRole !== 'admin') {
          showError('Esta página es solo para empleados y administradores');
          router.push('/');
          return;
        }
      } catch (error) {
        console.error('Error al verificar el acceso:', error);
        showError('No se pudo verificar el acceso');
        router.push('/');
      }
    };
  ...
  const fetchAppointments = useCallback(async (force = false) => {
    ...
    if (!force && appointments.length > 0 && now - lastFetch < 5 * 60 * 1000) {
      console.log("🔄 [Admin] Usando datos en caché...");
      return;
    }

    try {
      setLoading(true);
      console.log("🔄 [Admin] Actualizando citas desde el servidor...");
      ...
      // Filtrar las citas que corresponden al empleado actual
      const employeeAppointments = appointmentsData.filter(appointment => {
        return appointment.employee?.id?.toString() === userId;
      });

      // Obtener la información de los clientes
      const appointmentsWithClientInfo = await Promise.all(
        employeeAppointments.map(async (appointment) => {
          try {
            if (appointment.client?.id) {
              const clientResponse = await api.get(`/clients/${appointment.client.id}`);
              return {
                ...appointment,
                client: {
                  ...appointment.client,
            ...
            console.error(`Error al obtener información del cliente ${appointment.client?.id}:`, error);
            return {
              ...appointment,
              client: {
                ...appointment.client,
      ...
      const sortedAppointments = appointmentsWithClientInfo.sort((a, b) => {
      ...
      showAlert({
        title: 'Error',
        message: error instanceof Error ? error.message : 'No se pudieron cargar las citas'
      });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  }, [appointments.length, lastFetch, showAlert]);
  ...
  // Actualizar cuando la pantalla recibe el foco
  useFocusEffect(
    useCallback(() => {
  ...
  const updateAppointmentStatus = async (appointmentId: number, newStatus: AppointmentStatus) => {
    try {
      ...
      showError('No se pudo actualizar el estado de la cita');
    }
  };
  ...
  const handleRefreshPress = useCallback((event: GestureResponderEvent) => {
    onRefresh();
  }, [onRefresh]);

  const handleStatusFilterPress = useCallback((event: GestureResponderEvent) => {
    setStatusFilter(null);
  }, []);

  const handleFilterPress = useCallback((status: string) => (event: GestureResponderEvent) => {
    setStatusFilter(status);
  }, []);

  const handleEmptyStateRefresh = useCallback((_event: GestureResponderEvent) => {
    void onRefresh();
  }, [onRefresh]);
